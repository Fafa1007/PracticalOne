...
```{r}
flights <- as.tibble(nycflights13::flights)
flights
```
...
flights |> filter (carrier=="OO", month == 1) |> select(carrier, distance) |> distinct()
...
1.  **Height:** has a missing N/A value.
2.  **Disease_status:** "Healthy" spelt wrong for one of the entries. Should be "healthy".
3.  **Glucose:** has a missing N/A value.
